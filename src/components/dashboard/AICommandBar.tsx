    ...
    <div className="space-y-3">
      <form onSubmit={handleSubmit} className="relative">
        ...
        <input
          ...
          disabled={isLoading}
          className="w-full h-11 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-xl pl-11 pr-14 text-[15px] text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-[var(--accent)]/45 focus:ring-2 focus:ring-[var(--accent)]/10 hover:border-[var(--border-secondary)] transition-all duration-300 disabled:opacity-50 shadow-sm"
        />
        <div className="absolute right-4 top-1/2 -translate-y-1/2">
          {isLoading ? (
            <Loader2 className="w-4 h-4 text-[var(--text-quaternary)] animate-spin" />
          ) : (
            <kbd className="hidden sm:inline-flex items-center px-1.5 py-0.5 bg-[var(--bg-hover)] border border-[var(--border-primary)] rounded-md text-[16px] font-semibold text-[var(--text-quaternary)]">
