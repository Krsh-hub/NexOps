    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-[1400px]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[13px] text-[var(--text-tertiary)] mt-1">{vendors.length} registered suppliers</p>
        </div>
      ...
      {vendors.length === 0 ? (
        <div className="card-surface p-16 text-center">
          ...
          <p className="text-[13px] text-[var(--text-quaternary)]">No vendors yet</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {vendors.map((v: any) => (
            <div key={v.id} className="card-elevated p-5 flex flex-col gap-3">
              <div>
                <h3 className="text-[14px] font-semibold text-[var(--text-primary)]">{v.name}</h3>
                {v.contactPerson && <p className="text-[12px] text-[var(--text-tertiary)] mt-0.5">{v.contactPerson}</p>}
              </div>
