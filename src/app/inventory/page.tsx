          ...
          <thead>
            <tr className="border-b border-[var(--border-primary)]">
              {["Product", "SKU", "Vendor", "Stock", "Threshold", "Status"].map((h) => (
                <th key={h} className="text-left px-4 py-3 text-[15px] font-semibold text-[var(--text-quaternary)] uppercase tracking-wider first:rounded-tl-[var(--radius)] last:rounded-tr-[var(--radius)] last:text-center">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--border-primary)]">
            {products.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-16 text-[15px] text-[var(--text-quaternary)]">
              ...
              return (
                <tr key={p.id} className="hover:bg-[var(--bg-hover)] transition-colors duration-150">
                  <td className="px-4 py-3.5 text-[15px] font-medium text-[var(--text-primary)]">{p.name}</td>
                  <td className="px-4 py-3.5 text-[15px] text-[var(--text-tertiary)] font-mono">{p.sku}</td>
                  <td className="px-4 py-3.5 text-[15px] text-[var(--text-secondary)]">{p.vendor?.name || "—"}</td>
                  <td className="px-4 py-3.5 text-[15px] font-semibold text-[var(--text-primary)]">{p.currentStock} {p.unit}</td>
                  <td className="px-4 py-3.5 text-[15px] text-[var(--text-tertiary)]">{p.reorderThreshold}</td>
                  <td className="px-4 py-3.5 text-center">
                    <span className={`inline-block text-[16px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${
